...
int main(int argc, char *argv[]) {
  ...
  printf("=== 2 ptrs malloc'd ===\n");
  void *ptr0 = ics_malloc(64);
  void *ptr1 = ics_malloc(37);
  press_to_cont();
  
  printf("=== ptr0 freed, data copied into ptr1 ===\n");
  strncpy(ptr1, "Ravioli ravioli give me the formuoli", 37);
  ics_free(ptr0);
  ics_freelist_print_compact();
  press_to_cont();

  printf("=== ptr2 reallocs ptr1 ===\n");
  void *ptr2 = ics_realloc(ptr1, 77);
  ics_freelist_print_compact();
  ...
  // Test #2: Allocate a long double
  
  int *value3 = ics_malloc(sizeof(long double));
  null_check(value3, sizeof(long double));
